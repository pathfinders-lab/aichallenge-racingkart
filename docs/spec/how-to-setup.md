...
### (D) GitHub CLI 認証を行う（ダッシュボード即時反映に必要）

`make trial` 完了後、結果を即座にダッシュボードへ反映するには `gh` CLI の認証が必要です。

```bash
gh auth login
```

- ブラウザで GitHub にログインして認証を完了してください
- 認証済みの場合: `make trial` 終了直後にダッシュボードが更新されます
- **未認証でも動作します**（1時間以内に自動同期されます）

完了の目安: `gh auth status` でアカウント名が表示される。
