...
a_max:           1.37 m/s²  # 最大加速度 ⚠️ 7/1版バイナリでは実加速+0.76となり、旧縦モデル前提の現行軌道では s≈199/280 で壁接触を誘発（issue #67。a_max=1.0なら接触ゼロだが47.0s。恒久対応は縦モデル再同定 mpc#46）
...
**⚠️ 7/1版バイナリでのプラント変更（2026-07-08、issue #67 で確定）:** 加速度クランプが公式値 ±1.37 まで実際に効くようになり（5/31版は ±1.0 飽和）、低速抵抗も増加（指令 1.0 の実加速 +0.73→+0.48）、制動も強化（指令 -1.37→実 -1.48、-2.5→-1.55 飽和）。壁・コライダー地図は不変（両バイナリ抽出でセル単位一致）。**5/31版で同定した縦モデル・それを前提とした現行 trajectory・上表 41.6s はいずれも 7/1 版では成立しない。** 再同定 → trajectory 再生成は `multi_purpose_mpc_ros_custom` issue #46。
...
3. **加速度バウンドの clamp 整合: 7/1版で前提崩壊（issue #67）**。5/31版の実測（issue #53: 指令 ±約1.0 で飽和、a_max=1.37 は「飽和張り付き」効果で −1.36 s/lap）は旧バイナリ限定。7/1版のステップ実験（`output/step_exp/v0701_*.csv`）では **指令 1.0→実 +0.48 / 1.37→+0.76 / 2.0→+0.76（1.37 で飽和）、制動 -1.37→実 -1.48 / -2.5→-1.55**。a_max=1.37 は実加速になり、旧縦モデル前提の現行軌道ではコーナー立ち上がりで膨らんで（hotspot |e_y| 0.6–0.8 → 0.9–1.9）計画クリアランス 0.5 m の s≈199/280 で壁接触する。a_max=1.0 に戻すと接触ゼロだが 47.0 s（プロファイル自体の失効を示す）。恒久対応は mpc issue #46（再同定 → 再生成）。バイナリ更新時のステップ再実験手順は `output/step_exp/step_input_node.py`
...
| 加速度 clamp 実測（issue #53） | a_max 1.0→1.37 の余地と a_min 実効値の確認 | 43.0→41.6s（−1.36s/lap）・衝突ゼロ | — | **完了・ただし 5/31 版限定**（mpc PR #38。7/1 版で前提崩壊 → issue #67/§2.6） |
