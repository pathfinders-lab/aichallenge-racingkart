...
```
env/final_ver4/traj_mincurv.csv
```
...
| ウェイポイント数 | 349点 |
| 平均間隔 | 約1.0 m（実測値。旧記載の「約0.6m」は誤りだったため訂正） |
| 占有格子地図 | `env/final_ver4/occupancy_grid_map.yaml`（0.1 m/セル解像度） |
...
```yaml
reference_path:
  csv_path: "env/final_ver4/traj_mincurv.csv"  # ← ここを変更するだけ
```

> **制約:** 差し替える CSV の waypoint 間隔は約1.0 m を維持すること。§7.1 のウィンドウ探索（`search_window=30`）と N=20 の先読み距離（約20 m）はこの間隔を前提に検証されている。
...
#### 主要ボトルネック（2026-07-06更新）

究極設定適用後の実測（best_lap ≈ 51.3 s）に対し、全周を v_max=30 km/h で走り切った場合の理論下限は 347 m ÷ 8.33 m/s ≈ **41.7 s**。失われている約10秒はすべてコーナー区間に由来する。§3.1 の感度分析により、残るボトルネックは以下の2つに絞り込まれた:

1. **ヘアピン（wp223, 旋回半径2.25 m）でのステア動特性** → `ay_max` を24まで上げても改善しないことから、コーナリング速度の許可値ではなくステアの物理的な追従が律速
2. **コーナー立ち上がりの加速の遅さ**（`a_max=0.7` による事前速度プロファイルの制限）→ 単純に上げると衝突するため、縦方向制御の構造改善が必要（§3.6）
...
| `a_max` | 0.8 (+14%) | **衝突。** コーナー立ち上がりで目標速度が急上昇し、ステア戻し中に加速して壁に接触 |

**読み取れること:**

- `ay_max` は 14.4〜18 程度への引き上げが安全なフリーランチ（約0.7s/周）。それ以上は無意味
- `a_max` はコーナー立ち上がりの損失（数秒/周）を握る本命レバーだが、**単独で上げると危険**。`a_max` は毎ティックのMPC制約ではなく、起動時に1回だけ計算される参照速度プロファイル（`compute_speed_profile()`）の平滑化にのみ使われており、「ステアを切っている最中かどうか」を知らないため（対策は §3.6）
- ステアレート制約はリアルタイムMPCに存在する（`steering_rate_matrix`）が、**速度側の同等の制約は存在しない**という非対称がある（対策は §3.6）
...
**策定（2026-07-06）。未実装。** §3.1 の感度分析が示した通り、上限速度が低く（30 km/h、実車公式上限）カーブの多い本コースでは、直線は全車が速度上限に張り付くため、**タイム差はコーナー区間（進入・旋回・立ち上がり）だけで生まれる**。理論下限41.7 s に対し現状51.3 s であり、残る約10秒を削るには単一パラメータの調整ではなく、以下の3段階の構造的な施策を順に適用する。

#### B. mintimeライン + vx_mps 活用（§3.2 / §3.3 の実行）

`osm-to-raceline` で GGV 制約下の時間最小ラインを生成し、その `vx_mps` 列を `load_ref_path()` で読み込んで参照速度に使う。動力学的に正しい速度プロファイルが得られるため、粗い `compute_speed_profile()`（`a_max`/`a_min` の単純な定数制約）への依存が減る。**waypoint間隔は約1.0 m を維持すること**（§2.3 の制約参照）。

#### C. 摩擦円（楕円）カップリング

`compute_speed_profile()` の縦加速度制約を、固定値 `a_max` から曲率依存のベクトルに変える（楕円近似: `ax_budget[i] = a_max · √(1 − (ay[i]/ay_max)²)`、`ay[i]` は各waypointの曲率ベース旋回加速度から推定）。「横Gを使っている最中は縦の加速枠が自動的に減り、ステアが戻るにつれて枠が開く」挙動になり、`a_max=0.8` で起きた「ステア戻し中の急加速→壁接触」のメカニズムを構造的に防ぎながら、立ち上がりの加速をコーナー脱出後に使い切れるようになる。公式の加速度上限（約1.0 m/s²）の範囲内でも効果がある。

#### D. 速度レート制約の追加

`_init_problem()` に、既存のステアレート制約行列（`steering_rate_matrix`）と対になる形で `|v[k+1] − v[k]| ≤ a_limit · Ts` の制約行（N−1 行）を追加する。現状、リアルタイムMPCには速度変化の急さを直接抑える仕組みがなく（ステア側にはある）、縦の滑らかさの保証が「事前計算した `v_ref` の形」だけに依存している非対称を解消する。
...
| タスク | 内容 | 期待効果 | 優先度 | 状況 |
|-------|------|---------|--------|------|
| **B: mintime 軌跡生成 + vx_mps 活用（§3.6）** | `osm-to-raceline` で生成し、速度プロファイルも読み込む | コーナー区間の構造的短縮 | **最高** | ツールは構築済み・実行は未着手（§3.2/3.3） |
| **C: 摩擦円カップリング（§3.6）** | `compute_speed_profile` の a_max を曲率依存化 | 立ち上がり加速の安全な解放（数秒/周級） | 高 | 未着手（B の後） |
| **D: 速度レート制約（§3.6）** | `_init_problem` に縦方向のレート制約行を追加 | リアルタイム側の縦平滑性保証 | 高 | 未着手（C の後） |
| ay_max 引き上げ | 12.0 → 14.4〜18（感度分析で安全確認済み） | 約0.7s/周 | 中 | 6周 trial での再検証待ち |
| アイテム収集 | `/aichallenge/objects` からニトロを取得 | ブースト活用 | 中 | 未着手 |
| MPCパラメータ Optuna 最適化 | ラインを固定し30〜50 trial 実行 | MPC パラメータ収束 | 中 | 基盤完了。B〜D 適用後の再チューニングとして実行 |
| 動的 wp_id_offset | タイムスタンプベース実装 | 環境差の根本解決 | 高 | 未着手 |

**優先順位の考え方（2026-07-06更新）:**

```
1位: 縦方向制御の改善 B → C → D（§3.6）
     → 感度分析により単一パラメータ調整は頭打ちと判明。残る約10秒はここにしかない
2位: ay_max の引き上げ確定（6周 trial で再検証してから 14.4〜18 に）
3位: 環境差対応（動的 wp_id_offset）→ 提出前に必ず完成させる
4位: B〜D 適用後の Optuna 再チューニング
