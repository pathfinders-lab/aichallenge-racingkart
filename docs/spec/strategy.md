...
- **AWSIM 入力段で加速度は ±1.37 m/s² に clamp** される。1.37〜3 m/s² の入力はペナルティにならないが効かない「実質無意味」な入力（→ a_max の合法上限は 1.37）。**実測（issue #53）ではプラント正味加速度は指令 ±約1.0 で飽和**しており、±1.37 指令の効果はローパス後指令の飽和張り付き（§2.2）
...
a_min:          -1.37 m/s²  # 最大減速度（AWSIM入力clampに整合、mpc PR #38。実測の正味減速は≈-1.0で飽和、旧-2.5は実挙動の約2.5倍を仮定していた）
a_max:           1.37 m/s²  # 最大加速度（AWSIM入力clampに整合、mpc PR #38。実測の正味加速は指令≈+1.0で飽和するが、ローパス後指令の飽和張り付きで-1.36s/lap。issue #53）
...
- `teleop_manager` に手動発動用 `publish_turbo()` が実装済み（ジョイスティック用）
- `mpc_controller` は `/awsim/status` の index 1/2（ラップ数・タイム）のみ消費しており、index 5/6 は未活用
- 修理アイテムは 2026 年未実装。加速アイテムは開始時から保有する扱いで、コース上での収集は不要の可能性が高い
...
| **加速度バウンドのclamp整合 ±1.37（PR #38、現在）** | **≈41.6 s**（best 41.395 s / avg2to6 41.559 s） | **ゼロ（6周trial 1回クリーン）** |
...
3. **加速度バウンドの clamp 整合: 解消済み**（issue #53 / mpc PR #38）。ステップ入力実験でプラント正味加速度は指令 ±約1.0 で飽和と実測（加速: 指令1.0/1.2/1.37/2.0 で dv/dt 完全一致、ブレーキ: -1.37/-2.0/-2.5 で実減速≈-1.0 に一致）。a_max=1.37 はローパス後指令の飽和張り付きで有効（−1.36 s/lap、6周trialペナルティ0）、a_min=-1.37 は -2.5 の過大評価を解消（ラップタイム中立、mintime軌道の想定減速-0.88が実効内のため）。AWSIMバイナリ更新時は `output/step_exp/step_input_node.py` で再実験する
...
| ステップ | 内容 | 難易度 |
|---|---|---|
| 1 | `/awsim/cmd` 手動発動での実挙動確認（+0.5 m/s²・10 秒・残回数 index 5 の減り方） | 低 |
| 2 | 発動区間の設計: 加速区間・コーナー立ち上がりで使うのが定石。1 周 ≈ 44 s×6 周に対し計 50 秒分の配分 | 中 |
| 3 | 位置（s 座標）ベースの発動ノード実装 + 残回数（`/awsim/status` index 5）管理 | 中 |
| 4 | 縦制御との整合: ブースト中の +0.5 m/s² を速度プロファイル/MPC が活かせるか（現行プロファイルは非ブースト前提） | 中 |

**優先度:** 中期ロードマップ 1 位（§8.3）。
...
| 加速度 clamp 実測（issue #53） | a_max 1.0→1.37 の余地と a_min 実効値の確認 | 43.0→41.6s（−1.36s/lap）・衝突ゼロ | — | **完了**（mpc PR #38、§2.2/§2.6） |
...
```
1位: 公式ブースト発動戦略（issue #47）＋ 加速度 clamp 実測（issue #53）→ 合法的な縦加速の上積み
2位: 環境差対応（動的 wp_id_offset）→ 提出前に必ず完成させる
3位: 縦モデル再設計後の Optuna 再チューニング（§2.6・§3.6で完了した新プロファイルをベースに）
4位: 障害物回避の有効化検証（issue #9の相互回避スタックの解決含む）
```
