...
**策定（2026-07-06）。未実装。** §3.1 の感度分析が示した通り、上限速度が低く（30 km/h、実車公式上限）カーブの多い本コースでは、直線は全車が速度上限に張り付くため、**タイム差はコーナー区間（進入・旋回・立ち上がり）だけで生まれる**。理論下限41.7 s に対し現状51.3 s であり、残る約10秒を削るには単一パラメータの調整ではなく、以下の施策を順に適用する。各段階の効果測定を分離するため、1段階ごとに6周 `make trial` で検証する。
...
| タスク | 内容 | 期待効果 | 優先度 | 状況 |
|-------|------|---------|--------|------|
| **B-1: mintimeライン差し替え（§3.6）** | 検査済みCSV（width_opt=2.70版）に csv_path を変更 | 最難所の減速量激減・理論ラップ44.6s | **最高** | **CSV生成・検査済み**。6周 trial 待ち |
| **B-2: vx_mps 配管（§3.6）** | CSV の vx_mps 読み込み + v_ref 平坦化上書きの per-waypoint 化 | ホライゾン外コーナーへの事前減速 | **最高** | 未着手（B-1 の後） |
| **D: 速度レート制約（§3.6）** | `_init_problem` に縦方向のレート制約行を追加 | リアルタイム側の縦平滑性保証 | 高 | 未着手（B-2 の後） |
| a_max 引き上げ | 0.7 → 1.0（公式上限に一致、加速度指令クリップの解放） | 立ち上がり加速の改善 | 高 | B-1 適用後に6周 trial で検証 |
...
```
1位: 縦方向制御の改善 B-1 → B-2 → D（§3.6）
     → 感度分析により単一パラメータ調整は頭打ちと判明。残る約10秒はここにしかない
2位: a_max=1.0（公式上限）・ay_max 14.4〜18 の確定（B-1 適用後に6周 trial で1変更ずつ検証）
