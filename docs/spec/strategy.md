...
a_min:          -1.37 m/s²  # 最大減速度（AWSIM入力clampに整合。7/1版では実減速-1.48まで効く）
a_max:           1.37 m/s²  # 最大加速度（7/1版で入力clamp=実効値。旧軌道での壁接触は軌道とプラントのモデル乖離が原因で、7/1版再同定軌道では接触ゼロを確認済み。mpc#46）
traj_ay_budget:  24.0 m/s²  # effective_ax_limit() の摩擦円バジェット（mpc PR #54 でconfig化）。プラント限界ではなく「コーナー中の縦推力の自制度」ノブ: 12→24 で -2.0s/lap（詳細 §2.6 ボトルネック4）
...
- **加速度指令**: `effective_ax_limit()`（trajectoryの`ax_mps2`と摩擦円ヘッドルーム `a_max·√(1−(v²κ/traj_ay_budget)²)` のmax）でクリップし、コーナー中の全開加速を抑制。AWSIMのskid-cancelプラントには実際は摩擦円がなく（実測でヘアピン|ay|=12と駆動を併用可能）計画側もコーナー中フル駆動を仮定するため、これは安全側の自制ノブ。budget=12時代は7/1版の大抵抗と合わさり正味加速≈0でコーナー連鎖の速度不足（-4.1s/lap）の主因だった（mpc#46 の2026-07-08プローブで確定、指令は動的上限に96-99%張り付き）
...
| vx_mps | 8.58〜9.50 m/s（7/1版再同定モデルで再生成、`traj_mintime_awsim0701.csv`、mpc PR #47。計画上限9.5=7/1版の観測プラント最高速。計画ラップ36.19 s） |
...
| 属性 | 値 |
|------|----|
| 使用可能数 | **公式評価は 2 回**（eval イメージ内スクリプトの `--boosts 2` で確認。AWSIM 起動オプション `--boosts` で 0〜無限に設定可、ローカルの simulator_scripts も 2 に統一） |
| 効果 | **+0.5 m/s² を 10 秒間** |
...
| （7/1版バイナリ + 旧軌道 = プラント変更でモデル失効） | a_max=1.37 で12周中6回の壁接触 / a_max=1.0 で 47.0 s | issue #67 → mpc#46 で解消 |
| 7/1版再同定モデルの trajectory（mpc PR #47） | ≈41.7 s（avg2to6 41.716 / 41.740 s） | ゼロ（6周trial×2 + ペナルティ署名0） |
| **traj_ay_budget 12→24（mpc PR #54、現在）** | **≈39.7 s**（best 39.629 / avg2to6 39.729 s、マージ後再現 39.754 s） | **ゼロ（6周trial×2、ペナルティ署名0、min_wall 1.03-1.06 m）** |
| （参考）公式ブースト有効（#70、budget=12時の実測） | −0.45 s/lap（41.72→41.26 s）。budget=24 との複合は未計測 | ゼロ |
...
**7/1版バイナリでのプラント変更（issue #67 → mpc#46 で対応完了）:** 加速度クランプが公式値 ±1.37 まで実際に効くようになり（5/31版は ±1.0 飽和）、低速抵抗も増加、制動も強化。壁・コライダー地図は不変。7/1版でフル再同定した縦モデル（下記ボトルネック1）で trajectory を再生成（mpc PR #47、`traj_mintime_awsim0701.csv`）し、クリーン 41.7s を6周×2で再現。さらに律速だった `traj_ay_budget` の引き上げ（mpc PR #54）で **現在 39.7s**。バイナリ更新時のステップ再実験手順は `output/step_exp/step_input_node.py`（AWSIMはSharePoint同一URLで差し替え配布されるため、軌道・モデル・実測値には必ずビルド版を併記する）。

#### 主要ボトルネックと次の伸び代（2026-07-07更新）

1. **縦モデルの精度: 7/1版で再同定完了（mpc issue #46、2026-07-08）**。7/1版の実測: 駆動は入力クランプ +1.37 で飽和（f_drive=219.2 N）、抵抗 r(v) = 0.499 + 0.0410·v [m/s²]（F_res(v)=79.81+6.563·v N、クリーン trial 5506サンプル、rmse 0.127、有効域 0.3-8.5 m/s）、制動 net = -(1.44+0.040·v)（f_brake≈150 N + 抵抗の分解が実測傾きと一致）、観測プラント最高速 9.68 m/s（計画上限は 9.5 を採用）。osm-to-raceline に注入して trajectory 再生成（計画 vx 8.58-9.50、計画ラップ 36.19 s、o2r PR #4）。旧 5/31版の同定値（r=0.167+0.072v、駆動+1.0飽和）は失効・参考値
2. **公式ブースト: 配線・検証済み（#70、デフォルトoff）**: 5 回×10 秒×+0.5 m/s²（§2.5）。budget=12 時の実測で −0.45 s/lap（41.72→41.26 s、接触ゼロ×2）。**traj_ay_budget=24 との複合は未計測**（コーナー中の推力自制が緩んだ分、ブースト窓の効き方が変わる可能性。Optuna #48 前に 1 本確認推奨）。exploit 型 boost は OVER ペナルティで封殺済み（issue #46）
3. **加速度バウンドの clamp 整合: 解消（issue #67 → mpc#46）**。7/1版では ±1.37 が入力クランプ=実効値（指令 2.0 も 1.37 で飽和）。旧軌道での壁接触は a_max 単独ではなく「旧モデル前提の軌道とプラントの乖離」が原因で、再同定軌道では a_max=1.37 のまま接触ゼロ。a_min=-1.37 は実減速 -1.48 まで効く（据え置き）
4. **コーナー連鎖の速度不足（計画比 -4.1s）: 主因を特定し -2.0s 回収済み（mpc#46 の2026-07-08プローブ）**。s別分解で損失は s≈150-260 の中曲率帯に一様分布、加速度指令は動的上限 `effective_ax_limit` に全域 96-99% 張り付き = 摩擦円減額（traj_ay_budget=12）が律速だった。対照実験で R[0]（1e5→1e4）と計画v_max（9.5→10.2）は完全無効と実証。budget 12→15→18→24 = 41.6→40.7→40.1→39.6s（ペナルティ0維持）で、**24 を採用（mpc PR #54、6周×2で39.7s持続）**。残る計画比 -3.5s の構造対策は計画側モデルへの横G減額注入（osm-to-raceline#5、ラインごと再最適化）。過去の残余候補だった steer-rate 律速・weaving は本プローブでは主因ではなかった（横限界はヘアピンで|ay|=12まで実際に使用可能と実測）
