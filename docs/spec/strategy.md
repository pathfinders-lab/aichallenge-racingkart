...
- **AWSIM 入力段で加速度は ±1.37 m/s² に clamp** される。1.37〜3 m/s² の入力はペナルティにならないが効かない「実質無意味」な入力（→ a_max の合法上限は 1.37。実測検証は issue #53）
...
a_max:           1.0 m/s²   # 最大加速度（AWSIM入力clampは±1.37=合法上限、±3超はOVERペナルティ。1.37への引き上げ余地は issue #53 で検証）
...
| 加速度 clamp 実測（issue #53） | a_max 1.0→1.37 の余地と a_min 実効値の確認 | 合法的な縦加速の上積み | 高 | 未着手（§1.4） |
