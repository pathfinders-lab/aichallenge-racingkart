...
7. [ロバスト性・信頼性](#7-ロバスト性信頼性)
   - 7.1 ハードウェア差への対応（動的 wp_id_offset）
   - 7.2 計算時間の監視とフォールバック
   - 7.3 make eval での最終検証フロー
...
MPCが使用する列は `x_m, y_m, psi_rad, kappa_radpm` の4列のみ。`vx_mps` および `ax_mps2` は現状無視されており、速度プロファイルはMPC側の `v_max` / `ay_max` パラメータで決定される。
...
**最優先: 評価ループの確立**（他の全タスクに乗数効果がある）

| タスク | 内容 | 優先度 |
|-------|------|--------|
| `analyze_rosbag.py` | rosbag → JSON メトリクス変換 | **最高** |
| `generate_dashboard.py` | JSON → Plotly HTML | **最高** |
| 起動時ベンチマーク | solve_time 計測 → offset 自動設定 | 高 |
| Optuna 連携 | ローカルベイズ最適化の設定 | 高 |

`analyze_rosbag.py` が出力すべき最低限の JSON:

```json
{
  "lap_times": [64.2, 60.1, 59.8],
  "avg_solve_ms": 12.3,
  "collision_count": 0,
  "infeasible_count": 2,
  "avg_speed_kmh": 21.4
...
| タスク | 内容 | 期待効果 | 優先度 |
|-------|------|---------|--------|
| mintime 軌跡生成 | TUMFTM で final コース用を生成 | レーシングライン最適化 | 高 |
| vx_mps 活用 | TUMFTM の速度プロファイルを MPC に渡す | 速度プロファイル改善 | 中 |
| アイテム収集 | `/aichallenge/objects` からニトロを取得 | ブースト活用 | 中 |
| Optuna 最適化 | ローカルで 30〜50 trial 実行 | MPC パラメータ収束 | 高 |
| 動的 wp_id_offset | タイムスタンプベース実装 | 環境差の根本解決 | 高 |

**優先順位の考え方:**

```
1位: 評価インフラ（ダッシュボード・Optuna）→ 他全てに乗数効果
2位: 速度改善（パラメータ最適化・mintime 軌跡）→ ラップタイムに直結
